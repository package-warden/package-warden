...
public class PyPIProxyTests
{
    private readonly FakeRegistryContainer _registry;

    public PyPIProxyTests(FakeRegistryContainer registry)
    {
        _registry = registry;
    }

    [Fact]
    public async Task SimpleIndex_Returns200_WithPackageLinks()
    {
        using var factory = new PackageWardenFactory(_registry.BaseUrl) { EnabledEcosystem = "pypi" };
        using var client = factory.CreateClient();

        var response = await client.GetAsync("/v1/proxy/pypi/simple/test-package/");
    ...
    public async Task TarballDownload_Returns200_WhenNoBlockingPolicy()
    {
        ...
        var response = await client.GetAsync("/v1/proxy/pypi/packages/ab/test-package/test-package-1.0.0.tar.gz");
    ...
    public async Task TarballDownload_RecordsAllowedRequest_InStateStore()
    {
        ...
        await client.GetAsync("/v1/proxy/pypi/packages/ab/test-package/test-package-1.0.0.tar.gz");
